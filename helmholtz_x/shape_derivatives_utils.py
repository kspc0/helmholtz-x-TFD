...
# normalizing shape derivatives for nonaxisymmetric case
...
# This class generates a rectangular FFD Lattice
class FFDRectangular:
    def __init__(self, gmsh_model, l, m, dim, tag, includeBoundary, parametric):
        self.l = l # x direction
        self.m = m # y direction
        # FFD points coordinates
        self.Px = np.zeros((l,m))
        self.Py = np.zeros((l,m))
        self.Pz = np.zeros((l,m))
        # get coordinates of nodes in the physical entity of dim and tag
        nodes, coords, param = gmsh_model.mesh.getNodes(dim, int(tag), includeBoundary, parametric)
        # read x and y coordinates
        xs = coords[0::3] # every third element
        ys = coords[1::3] # every third element starting from 1
        # print("xs", len(xs))
        # print("ys", len(ys))
        # dimensions of rectangle (decided by nodes)
        self.dx = max(xs)-min(xs)
        self.dy = max(ys)-min(ys)
        #self.dz = 0 # not needed
        # distribute number of points on the lattice
        for i in range(l):
            for j in range(m):
                self.Px[i, j] = min(xs)  + self.dx * i / (l - 1)
                self.Py[i, j] = min(ys)  + self.dy * j / (m - 1)
        # set the origin (first point)
        self.P0 = np.array([self.Px[0, 0], self.Py[0, 0]])
        # print("P0", self.P0)

    # save FFD points as a vtu file
    ...
    # calculate parametric coordinates for rectangular lattice
    def calcSTU(self, coords):
        """Calculates parametric coordinates for rectangular lattice
        Args:
...
### CREATE FFD LATTICE OF CONTROL POINTS
class FFDCylindrical:
    ...
    def __init__(self, gmsh_model, l, m , n, dim, tag=-1, includeBoundary=False, parametric=True):
        ...
        xs = coords[0::3] # every third element
        ys = coords[1::3] # every third element starting from 1
        zs = coords[2::3] # every third element starting from 2

        rhos, phis, zetas = cart2cyl(xs, ys, zs)

        # dimensions of total lattice?
...
## used to deform the gmsh model with parameter dependence on the FFD lattice
